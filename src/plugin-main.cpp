...
bool obs_module_load(void)
{
	try {
		...
		streamdeck::message(streamdeck::log_level::LOG_INFO,
				    "Plugin version %s", STREAMDECK_VERSION);

		obs_frontend_add_tools_menu_item(
			obs_module_text("Streamdeck.Title"),
			plugin_show_details, nullptr);

		_server = streamdeck::server::instance();
		_handler_system = streamdeck::handlers::system::instance();
		_handler_obs_frontend =
			streamdeck::handlers::obs_frontend::instance();
		_handler_obs_scene =
			streamdeck::handlers::obs_scene::instance();
		_handler_obs_source =
			streamdeck::handlers::obs_source::instance();
		return true;
	} catch (...) {
