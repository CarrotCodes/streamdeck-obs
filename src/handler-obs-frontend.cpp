...
static std::function<void(std::weak_ptr<void>,
			  std::shared_ptr<streamdeck::jsonrpc::request>)>
make_async(obs_task_type task_type,
		   ...
		   callback)
{
	...
	return [callback](std::weak_ptr<void> handle,
		...
		obs_queue_task(
			...
			[](void *ptr) {
				DLOG(LOG_DEBUG, "ran make async task");
				nlohmann::json params;

				// Retrieve passed things and clean up immediately.
				passed_data *pd =
					static_cast<passed_data *>(ptr);
				...
				auto res = std::make_shared<
					streamdeck::jsonrpc::response>();
				res->copy_id(*req);

				callback(req, res);

				streamdeck::server::instance()->reply(handle,
								      res);
			},
			pd, true);
	};
...
std::shared_ptr<streamdeck::handlers::obs_frontend>
streamdeck::handlers::obs_frontend::instance()
{
	...
	if (_instance.expired()) {
		auto instance =
			std::make_shared<streamdeck::handlers::obs_frontend>();
		_instance = instance;
		return instance;
	}
...
streamdeck::handlers::obs_frontend::~obs_frontend()
{
	obs_frontend_remove_event_callback(
		streamdeck::handlers::obs_frontend::handle_frontend_event,
		this);
}
...
streamdeck::handlers::obs_frontend::obs_frontend()
{
	...
	server->handle_sync(
		...
		std::bind(&streamdeck::handlers::obs_frontend::streaming_start,
			  this, std::placeholders::_1, std::placeholders::_2));
	server->handle_sync(
		"obs.frontend.streaming.stop",
		std::bind(&streamdeck::handlers::obs_frontend::streaming_stop,
			  this, std::placeholders::_1, std::placeholders::_2));
	server->handle_sync(
		"obs.frontend.streaming.active",
		std::bind(&streamdeck::handlers::obs_frontend::streaming_active,
			  this, std::placeholders::_1, std::placeholders::_2));

	server->handle_sync(
		"obs.frontend.recording.start",
		std::bind(&streamdeck::handlers::obs_frontend::recording_start,
			  this, std::placeholders::_1, std::placeholders::_2));
	server->handle_sync(
		"obs.frontend.recording.stop",
		std::bind(&streamdeck::handlers::obs_frontend::recording_stop,
			  this, std::placeholders::_1, std::placeholders::_2));
	server->handle_sync(
		"obs.frontend.recording.active",
		std::bind(&streamdeck::handlers::obs_frontend::recording_active,
			  this, std::placeholders::_1, std::placeholders::_2));

	server->handle_sync(
		"obs.frontend.recording.pause",
		std::bind(&streamdeck::handlers::obs_frontend::recording_pause,
			  this, std::placeholders::_1, std::placeholders::_2));
	server->handle_sync(
		"obs.frontend.recording.unpause",
		std::bind(&streamdeck::handlers::obs_frontend::recording_unpause,
			  this, std::placeholders::_1, std::placeholders::_2));
	server->handle_sync(
		"obs.frontend.recording.paused",
		std::bind(&streamdeck::handlers::obs_frontend::recording_paused,
			  this, std::placeholders::_1, std::placeholders::_2));

	server->handle_async(
		"obs.frontend.replaybuffer.enabled",
		make_async(obs_task_type::OBS_TASK_UI,
			   std::bind(&streamdeck::handlers::obs_frontend::
					     replaybuffer_enabled,
	...
	server->handle_async(
		...
		std::bind(&streamdeck::handlers::obs_frontend::studiomode, this,
			  std::placeholders::_1, std::placeholders::_2));
	server->handle_async(
		"obs.frontend.studiomode.enable",
		std::bind(&streamdeck::handlers::obs_frontend::studiomode_enable,
			  this, std::placeholders::_1, std::placeholders::_2));
	server->handle_async(
		"obs.frontend.studiomode.disable",
		std::bind(
			&streamdeck::handlers::obs_frontend::studiomode_disable,
			this, std::placeholders::_1, std::placeholders::_2));
	server->handle_async(
		"obs.frontend.studiomode.active",
		std::bind(&streamdeck::handlers::obs_frontend::studiomode_active,
			  this, std::placeholders::_1, std::placeholders::_2));

	server->handle_async(
		"obs.frontend.transition_studio",
		std::bind(&streamdeck::handlers::obs_frontend::transition_studio,
			  this, std::placeholders::_1, std::placeholders::_2));

	server->handle_async(
		"obs.frontend.scenecollection",
		std::bind(&streamdeck::handlers::obs_frontend::scenecollection,
			  this, std::placeholders::_1, std::placeholders::_2));
	server->handle_sync(
		"obs.frontend.scenecollection.list",
		std::bind(
			&streamdeck::handlers::obs_frontend::scenecollection_list,
			this, std::placeholders::_1, std::placeholders::_2));

	server->handle_async(
		"obs.frontend.scene",
		std::bind(&streamdeck::handlers::obs_frontend::scene, this,
			  std::placeholders::_1, std::placeholders::_2));
	server->handle_sync(
		"obs.frontend.scene.list",
		std::bind(&streamdeck::handlers::obs_frontend::scene_list, this,
			  std::placeholders::_1, std::placeholders::_2));

	server->handle_async(
		"obs.frontend.transition",
		std::bind(&streamdeck::handlers::obs_frontend::transition, this,
			  std::placeholders::_1, std::placeholders::_2));
	server->handle_async(
		"obs.frontend.transition.list",
		std::bind(&streamdeck::handlers::obs_frontend::transition_list,
			  this, std::placeholders::_1, std::placeholders::_2));

	server->handle_sync(
		"obs.frontend.screenshot",
		std::bind(&streamdeck::handlers::obs_frontend::screenshot, this,
			  std::placeholders::_1, std::placeholders::_2));
}

void streamdeck::handlers::obs_frontend::handle_frontend_event(
	enum obs_frontend_event event, void *private_data)
{
	static_cast<streamdeck::handlers::obs_frontend *>(private_data)
		->handle(event);
}
	...
	try {
		...
		case OBS_FRONTEND_EVENT_SCENE_LIST_CHANGED:
			...
			reply["scenes"] = nlohmann::json::array();
			{
				...
				for (size_t idx = 0; idx < list.sources.num;
				     idx++) {
					obs_source_t *source =
						*(list.sources.array + idx);
					const char *data =
		...
		case OBS_FRONTEND_EVENT_SCENE_COLLECTION_LIST_CHANGED:
			...
			reply["collections"] = nlohmann::json::array();
			{
				...
				for (char **ptr = collections; *ptr != nullptr;
				     ptr++) {
...
void streamdeck::handlers::obs_frontend::streaming_start(
	std::shared_ptr<streamdeck::jsonrpc::request>,
...
void streamdeck::handlers::obs_frontend::streaming_stop(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.streaming.stop
...
void streamdeck::handlers::obs_frontend::streaming_active(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.streaming.active
...
void streamdeck::handlers::obs_frontend::recording_start(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.recording.start
...
void streamdeck::handlers::obs_frontend::recording_stop(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.recording.stop
...
void streamdeck::handlers::obs_frontend::recording_active(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.recording.active
...
void streamdeck::handlers::obs_frontend::recording_pause(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.recording.pause
...
void streamdeck::handlers::obs_frontend::recording_unpause(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.recording.unpause
...
void streamdeck::handlers::obs_frontend::recording_paused(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.recording.paused
...
void streamdeck::handlers::obs_frontend::replaybuffer_enabled(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.replaybuffer.enabled
...
void streamdeck::handlers::obs_frontend::replaybuffer_start(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.replaybuffer.start
...
void streamdeck::handlers::obs_frontend::replaybuffer_save(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.replaybuffer.save
...
void streamdeck::handlers::obs_frontend::replaybuffer_stop(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.replaybuffer.stop
...
void streamdeck::handlers::obs_frontend::replaybuffer_active(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.replaybuffer.active
...
void streamdeck::handlers::obs_frontend::studiomode(
	std::weak_ptr<void> handle,
	std::shared_ptr<streamdeck::jsonrpc::request> req)
{
	/** obs.frontend.studiomode
	...
	obs_queue_task(
		obs_task_type::OBS_TASK_UI,
		[](void *ptr) {
			DLOG(LOG_DEBUG, "ran studiomode task");
			nlohmann::json params;
			...
			auto res =
				std::make_shared<streamdeck::jsonrpc::response>();
			res->copy_id(*req);
			res->set_result(
...
void streamdeck::handlers::obs_frontend::studiomode_enable(
	std::weak_ptr<void> handle,
	std::shared_ptr<streamdeck::jsonrpc::request> req)
{
	/** obs.frontend.studiomode.enable
...
void streamdeck::handlers::obs_frontend::studiomode_disable(
	std::weak_ptr<void> handle,
	std::shared_ptr<streamdeck::jsonrpc::request> req)
{
	/** obs.frontend.studiomode.disable
...
void streamdeck::handlers::obs_frontend::studiomode_active(
	std::weak_ptr<void> handle,
	std::shared_ptr<streamdeck::jsonrpc::request> req)
{
	/** obs.frontend.studiomode.active
...
void streamdeck::handlers::obs_frontend::transition_studio(
	std::weak_ptr<void> handle,
	std::shared_ptr<streamdeck::jsonrpc::request> req)
{
	/** obs.frontend.transition_studio
	...
	obs_queue_task(
		obs_task_type::OBS_TASK_UI,
		[](void *ptr) {
			DLOG(LOG_DEBUG, "ran transition studio task");
			// Retrieve passed things and clean up immediately.
			passed_data *pd = static_cast<passed_data *>(ptr);
			std::weak_ptr<void> handle = pd->handle;
			std::shared_ptr<streamdeck::jsonrpc::request> req =
			...
			try {
				...
				DLOG(LOG_ERROR, "Failed to send reply: %s",
				     ex.what());
			}
		},
		pd, true);
...
void streamdeck::handlers::obs_frontend::scenecollection(
	std::weak_ptr<void> handle,
	std::shared_ptr<streamdeck::jsonrpc::request> req)
{
	/** obs.frontend.scenecollection
	...
	if (req->get_params(params)) {
		...
		if (p != params.end()) {
			if (!p->is_string()) {
				throw jsonrpc::invalid_params_error(
					"Parameter 'collection' must be a string or String.");
			}
		}
	}
	...
	obs_queue_task(
		obs_task_type::OBS_TASK_UI,
		[](void *ptr) {
			DLOG(LOG_DEBUG, "ran scenecollection task");
			// Retrieve passed things and clean up immediately.
			passed_data *pd = static_cast<passed_data *>(ptr);
			std::weak_ptr<void> handle = pd->handle;
			std::shared_ptr<streamdeck::jsonrpc::request> req =
			...
			try {
				...
				if (params.size() > 0) {
					...
					if ((kv != params.end()) &&
						...
						throw streamdeck::jsonrpc::
							invalid_params_error(
								"'collection' must be a string value.");
					}
				}
...
void streamdeck::handlers::obs_frontend::scenecollection_list(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.scenecollection.list
...
void streamdeck::handlers::obs_frontend::scene(
	std::weak_ptr<void> handle,
	std::shared_ptr<streamdeck::jsonrpc::request> req)
{
	/** obs.frontend.scene
	...
	// Validate parameters.
	DLOG(LOG_DEBUG, "got scene request");
	nlohmann::json params;
	if (req->get_params(params)) {
		{
			...
			if (p != params.end()) {
				if (!p->is_boolean()) {
					throw jsonrpc::invalid_params_error(
						"Parameter 'program' must be a Boolean.");
				}
			}
		}

		{
			...
			if (p != params.end()) {
				if (!p->is_string()) {
					throw jsonrpc::invalid_params_error(
						"Parameter 'scene' must be a string or String.");
				}
			}
		}
	}
	...
	obs_queue_task(
		OBS_TASK_UI,
		[](void *ptr) {
			DLOG(LOG_DEBUG, "ran scene request task");
			...
			try {
				...
				DLOG(LOG_DEBUG, "replied with source name: %s",
				     name);
...
void streamdeck::handlers::obs_frontend::scene_list(
	std::shared_ptr<streamdeck::jsonrpc::request>,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.scene.list
...
void streamdeck::handlers::obs_frontend::transition(
	std::weak_ptr<void> handle,
	std::shared_ptr<streamdeck::jsonrpc::request> req)
{
	/** obs.frontend.transition
	...
	obs_queue_task(
		obs_task_type::OBS_TASK_UI,
		[](void *ptr) {
			DLOG(LOG_DEBUG, "ran scene transition task");
			nlohmann::json params;
			...
			try {
				...
				{
					auto source = std::shared_ptr<
						obs_source_t>(
						obs_frontend_get_current_transition(),
						[](obs_source_t *v) {
							obs_source_release(v);
						});
					auto name = obs_source_get_name(
						source.get());
					auto duration =
						obs_frontend_get_transition_duration();
				...
				streamdeck::server::instance()->reply(
					td->handle, res);
				...
				try {
					auto res = std::make_shared<
						streamdeck::jsonrpc::response>();
					res->copy_id(*td->req);
					res->set_error(jsonrpc::SERVER_ERROR,
						       ex.what());
					streamdeck::server::instance()->reply(
						td->handle, res);
				} catch (...) {
...
void streamdeck::handlers::obs_frontend::transition_list(
	std::weak_ptr<void> handle,
	std::shared_ptr<streamdeck::jsonrpc::request> req)
{
	/** obs.frontend.transition.list
	...
	obs_queue_task(
		obs_task_type::OBS_TASK_UI,
		[](void *ptr) {
			DLOG(LOG_DEBUG, "ran transition list task");
			auto td = reinterpret_cast<task_data *>(ptr);
			try {
				auto list = std::shared_ptr<
					obs_frontend_source_list>(
				...
				for (size_t idx = 0; idx < list->sources.num;
					...
					const char *name = obs_source_get_name(
						list->sources.array[idx]);
					bool fixed = obs_transition_fixed(
						list->sources.array[idx]);
					auto trans = nlohmann::json::object();
					trans["name"] = name ? name : "";
					trans["fixed"] = fixed;
...
void streamdeck::handlers::obs_frontend::screenshot(
	std::shared_ptr<streamdeck::jsonrpc::request> req,
	std::shared_ptr<streamdeck::jsonrpc::response> res)
{
	/** obs.frontend.screenshot
	...
	if (req->get_params(params)) { // Validate parameters.
		...
		if (param != params.end()) {
			if (!param->is_string()) {
				throw jsonrpc::invalid_params_error(
					"The parameter 'source' must be of type 'string' if provided.");
			} else {
				source = std::shared_ptr<obs_source_t>(
					obs_get_source_by_name(
